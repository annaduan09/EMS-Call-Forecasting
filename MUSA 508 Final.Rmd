...
In the field of Emergency Medical Services (EMS), response time is everything. When responding to EMS calls for conditions such as cardiac arrest or stroke, minutes can be the difference between life and death. Over the past year, the COVID-19 pandemic has pushed the efficiency and function of EMS around the country to the limit. Surges in EMS demand have caused ambulance delays and [shortages](https://www.startribune.com/fema-ambulances-deployed-to-help-transfer-minn-patients/573127151/) and the process of taking COVID-related protective measures such as donning personal protective equipment has increased response times. In Los Angeles County, paramedics were ordered to [delay transporting cardiac arrest patients](https://www.latimes.com/california/story/2020-04-04/l-a-county-911-patients-hospital-coronavirus) to the hospital until 5 minutes after they are revived in order to optimize use of limited ambulance and emergency room capacity.  
...
```{r Figure 1: EMS Call Density Map, message=FALSE, warning=FALSE, results='hide'}
...
ggplot() + 
  ...
  mapTheme(),
  ncol = 2,
  top = "Figure 1: Observed EMS Call Density in 2020 compared to past 10 years")
...
```{r Figure 4: risk factor scatterplots, fig.height=9, fig.width=8, message=FALSE, warning=FALSE, results='hide'}
...
The day of week with most calls is Tuesday    


```{r Figure 7: calls/day of week, fig.height=5, fig.width=8, message=FALSE, warning=FALSE, results='hide'}
ggplot(EMS_2020 %>%
         group_by(dotw) %>%
         tally())+
...
```{r Figure 8: EMS call by Rush, fig.height=5, fig.width=4, message=FALSE, warning=FALSE, results='hide'}
...
```{r Figure 10: weather conditions/time plot, fig.height=10, fig.width=8, message=FALSE, warning=FALSE, results='hide'}
library(ggplot2)
library(gridExtra)
#vsby: visibility; sknt:wind speed; gust:wind gust in knots
grid.arrange(
...
```{r Figure 15: corr plot, fig.height=6, fig.width=6, message=FALSE, warning=FALSE, results='hide'}
...
We evaluate the usefulness of these models using accuracy and generalizability as metrics. We test accuracy by calculating the Mean Absolute Error (MAE) in our predictions for each fishnet cell/hour pair. The lower the error, the more accurate our model. For generalizability, we check if our model predicts equally well across different spatial, temporal, and social contexts. For a use case that concerns saving lives and responding to time-sensitive emergencies, it is critical that our model predicts with equal accuracy across social contexts such as race, income, and health levels.    
...
Across our models, the lowest error is found in the temporal model (Figure 16). Our modelconsistently detects each demand peak, although it does not predict the full size of each peak every time (Figure 17). As a result, our average error is slightly high, as many space/time units have 0 observed calls.   
...
```{r Figure 17: Time series, fig.height=3, fig.width=6, message=FALSE, warning=FALSE, results='hide'}
...
```{r Figure 19: Error by fishnet grid, fig.height=2, fig.width=4, message=FALSE, warning=FALSE, results='hide'}
...
```{r Figure 20: MAE by time period, fig.height=3, fig.width=6, message=FALSE, warning=FALSE, results='hide'}
...
ggplot(MAE_plot_map)+
  geom_sf(data = ACS %>%
            st_transform(crs=4326), colour = 'white', fill = "gray90")+
  geom_sf(data = MAE_plot_map , aes(color = MAE),
             fill = "transparent", size = 2, alpha = 1.5)+
...
Finally, there is no significant correlation between EMS and percentage of residents who self report poor physical health. This is a very good sign. While income, age, and race can be proxies of health care needs and access, this outcome tells us that our model will not result in unequal EMS service for those who may need it most.   
...
# Cross Validation
#BC
In this section, goodness of fit metrics are generated for eight regressions - four including all Risk Factors (reg.vars), and two (reg.ss.vars) to include spatial risk factors plus the Local Moran's I Spatial Process, and the last two (reg.tt.vars) to include temporal risk factors. 
...
reg.ss.vars <- c("bar.nn", "downtown.nn", "Violent_Crime", "uniqueID", "EMS.isSig.x", "EMS.isSig.dist.x")
...
substudy.panel.net <- study.panel.net[,c("month", "h_interval60", "dotw", "Temperature", "Precipitation", "wind_gust", "Wind_Speed", "isWeekend", "isHoliday", "isPeakHour", "uniqueID", "Violent_Crime", "bar.nn", "downtown.nn", "lagHour", "lag2Hours", "lag4Hours", "lag12Hours", "lag1day", "EMS.isSig.x", "EMS.isSig.dist.x", "cvID", "EMS_Count", "geometry")]

sample <- sample_n(substudy.panel.net, 10000)
...
reg.cv <- crossValidate(
  ...
  indVariables = reg.vars) %>%
    dplyr::select(cvID = cvID, EMS_Count, Prediction, geometry)
...
reg.temcv <- crossValidate(
  ...
  dependentVariable = "EMS_Count",
  indVariables = reg.vars) %>%
    dplyr::select(cvID = month, EMS_Count, Prediction, geometry)
...
reg.spatialCV <- crossValidate(
  ...
  dependentVariable = "EMS_Count",
  indVariables = reg.vars) %>%
    dplyr::select(cvID = uniqueID, EMS_Count, Prediction, geometry)
...
reg.summary <- 
  rbind(
    ...
    mutate(reg.spatialCV,           Error = Prediction - EMS_Count,
                             Regression = "Spatial LOGO-CV: Just Risk Factors"),
                             
    mutate(reg.ss.spatialCV,        Error = Prediction - EMS_Count,
                             Regression = "Spatial LOGO-CV: Spatial Process"),
    
    mutate(reg.temporalCV,    Error = Prediction - EMS_Count,
                             Regression = "Spatial LOGO-CV: Just Risk Factors by Month"),
                             
    mutate(reg.ss.temporalCV, Error = Prediction - EMS_Count,
                             Regression = "Spatial LOGO-CV: Temporal Process by Month")) %>%
    st_sf() 
...
```{r CV figures, message=FALSE, warning=FALSE, results='hide'}
error_by_reg_and_fold %>%
  ggplot(aes(MAE)) + 
...
st_drop_geometry(error_by_reg_and_fold) %>%
  group_by(Regression) %>% 
    summarize(Mean_MAE = round(mean(MAE), 4),
              SD_MAE = round(sd(MAE), 4)) %>%
  kable() %>%
    kable_styling("striped", full_width = F) %>%
    row_spec(2, color = "black", background = "#FDE725FF") %>%
    row_spec(4, color = "black", background = "#FDE725FF") %>%
    row_spec(8, color = "black", background = "#FDE725FF") 
...
error_by_reg_and_fold %>%
  ...
  ggplot() +
    ...
    facet_wrap(~Regression) +
    scale_fill_viridis() +
    labs(title = "EMS calls errors by LOGO-CV Regression") +
    mapTheme() + theme(legend.position="bottom")

st_drop_geometry(reg.summary) %>%
  group_by(Regression) %>%
    mutate(EMS_Decile = ntile(EMS_Count, 10)) %>%
  group_by(Regression, EMS_Decile) %>%
    summarize(meanObserved = mean(EMS_Count, na.rm=T),
              meanPrediction = mean(Prediction, na.rm=T)) %>%
    gather(Variable, Value, -Regression, -EMS_Decile) %>%          
    ggplot(aes(EMS_Decile, Value, shape = Variable)) +
      geom_point(size = 2) + geom_path(aes(group = EMS_Decile), colour = "black") +
      scale_shape_manual(values = c(2, 17)) +
      facet_wrap(~Regression) + xlim(0,10) +
      labs(title = "Predicted and observed EMS calls by observed EMS_Decile")  +
      plotTheme()
